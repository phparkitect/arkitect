...
final class Implement implements Constraint
{
    ...
    public function evaluate(ParsedClass $class, ClassGraph $classGraph): Outcome
    {
        if (Depth::Direct === $this->depth) {
            return $this->declares($class)
                ? new Outcome()
                : $this->violation($class, \sprintf('does not directly implement %s', $this->target));
        }

        return match ($classGraph->isA($class->fqcn, $this->target)) {
            Membership::Yes => new Outcome(),
            Membership::No => $this->violation($class, \sprintf('does not implement %s', $this->target)),
    ...
    {
        foreach ($class->implements as $reference) {
    ...
    {
        return new Outcome(new Violations(Violation::create($class, self::class, $detail, $this->target)));
    }
}
