...
final class CheckClassHaveTraitTest extends TestCase
{
    ...
    {
        $fixturesDir = __DIR__.'/../Fixtures/HaveTrait/Feature';

        $runner = TestRunner::create('8.4');

        $rule = Rule::allClasses()
            ->that(new ResideInOneOfTheseNamespaces('Arkitect\Tests\Fixtures\HaveTrait\Feature'))
            ->should(new HaveTrait('Arkitect\Tests\Fixtures\HaveTrait\DatabaseTransactions'))
        ...
        self::assertEquals('Arkitect\Tests\Fixtures\HaveTrait\Feature\UserFeatureSpec', $runner->getViolations()->get(0)->getFqcn());
    }
    ...
    public function test_feature_tests_should_not_use_refresh_database_trait(): void
    {
        ...
        $rule = Rule::allClasses()
            ->that(new ResideInOneOfTheseNamespaces('Arkitect\Tests\Fixtures\HaveTrait\Feature'))
            ->should(new NotHaveTrait('Arkitect\Tests\Fixtures\HaveTrait\RefreshDatabase'))
        ...
        self::assertEquals('Arkitect\Tests\Fixtures\HaveTrait\Feature\ProductFeatureSpec', $runner->getViolations()->get(0)->getFqcn());
    }

    public function test_classes_with_uuid_should_have_uuid_trait(): void
    {
        $fixturesDir = __DIR__.'/../Fixtures/HaveTrait/Models';

        $runner = TestRunner::create('8.4');

        $rule = Rule::allClasses()
            ->that(new ResideInOneOfTheseNamespaces('Arkitect\Tests\Fixtures\HaveTrait\Models'))
            ->should(new HaveTrait('Arkitect\Tests\Fixtures\HaveTrait\HasUuid'))
